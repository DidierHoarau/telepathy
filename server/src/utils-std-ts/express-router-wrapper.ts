...
const logger = new Logger('express-router-wrapper');

export class ExpressRouterWrapper {
  //
  public static route(router: Router, method: string, path: string, handler: any): void {
    (router as any)[method](path, (req, res, next) => {
      let code = 500;
      let response = { error_code: 'SERVER_ERROR', error_message: 'unexpected error' };
